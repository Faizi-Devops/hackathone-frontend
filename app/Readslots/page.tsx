...
const Readslots = () => {
    ...
    const fetchData = async () => {
        try {
            setFlag(true);
            let response = await axios.get("http://localhost:4000/slot/read");
            setFlag(false);
    ...
    const onDeleteHandler = async (id: string) => {
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            try {
              await axios.delete(`http://localhost:4000/slot/delete/${id}`);
                                ...
                                {flag ? (
                                    ...
                                    data?.map((slot, index) => (
                                        ...
                                        >
                                            ...
                                            <td className="px-6 py-4">{slot?.interviewer}</td>
                                            <td className="px-6 py-4">
                                                <button
                                                    ...
                                                    className={`focus:outline-none text-white bg-red-700 hover:bg-red-800 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-900 ${
                                                        deletingId === slot._id ? "opacity-50 cursor-not-allowed" : ""
                                                    }`}
                                                >
                                                    {deletingId === slot._id ? (
                                                        <Circles
